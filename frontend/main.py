...
with col1:
    ...
    uploaded_file = form.file_uploader(label="Upload CV file",
                                   type=["pdf", "docx"],
                                   on_change=delete_folder_contents(resume_path),
                                   disabled=not st.session_state.valid_key,)
    output_type = form.radio(label="Select Output type", options=["Text Summary", "JSON Format"], horizontal=True)
    word_limit = form.slider(label="Select summary word limit", min_value=200, max_value=600, step=100, help="Only applicable for **Text Summary** option")
    ...
    if submit_button:
        ...
        if len(extracted_text) != 0:
            if output_type == "Text Summary":
                ...
                gpt_response = gpt.get_completion_from_messages(messages=prompt)
                ...
                prompt, cv_details_schema = extract_cv_details(resume_context=extracted_text, response_schema=response_schema)
                gpt_response = gpt.get_completion_from_messages(messages=prompt, functions=cv_details_schema)
