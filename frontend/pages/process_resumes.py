...
def process_resumes():
    ...
    if not st.session_state.valid_key:
        st.warning(
            "Invalid Open AI API Key. Please update your key and redeploy the app."
        )
    ...
    with col1:
        ...
        form.radio(
            label="Choose Run type",
            options=["Dry Run", "Live Run"],
            key="run_type",
            horizontal=True,
        )
        ...
        if submit_button:
            if st.session_state.run_type == "Dry Run":
                write_uploaded_files(
                    uploaded_files=uploaded_files, folder_path=resumes_path
                )
    ...
    with col2:
        with st.expander("", expanded=True):
            st.metric(
                ...
                value=count_files_in_directory(resumes_path),
            )
