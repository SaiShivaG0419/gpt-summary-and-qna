...
def input_documents():
    ...
    with st.form("Input_Documents"):
        uploaded_files = st.file_uploader(label="Upload documents",
                                   type=["pdf", "docx", "xlsx", "txt"],
                                   accept_multiple_files=True,
                                   disabled=not st.session_state.valid_key,)
        submit_button = st.form_submit_button(label="Upload Documents", disabled=not st.session_state.valid_key)
...
def input_url():
    ...
    with st.form("Input_WebURL"):
        input_url = st.text_input(label="Paste a Web URL",
                                    value='''https://en.wikipedia.org/wiki/Eiffel_Tower''')
        submit_url = st.form_submit_button(label="Extract Web Page Content")
        ...
        if submit_url:
            ...
            if validate_input_url(input_url):
                ...
                if len(extracted_text) == 0:
                    ...
                    db, db_build_time = vector_db.run_db_build(input_type="web_url", embeddings=gpt.embeddings, page_content=extracted_text, source_url=input_url, db_persist=True)
                    if db is not None:
...
def process_documents(persist_db: bool=True):
    ...
    """
    try:
        db, db_build_time = vector_db.run_db_build(input_type="documents", embeddings=gpt.embeddings, db_persist=persist_db)
        if db is not None:
            st.info(f"Database build completed in {db_build_time:.4f} seconds")
            st.session_state.db_exist = True
            return st.session_state.db_exist
...
def delete_vector_database():
    ...
    """
    try:
        drop_database = st.button(label="Reset Vector Database", use_container_width=True)
        if drop_database:
...
def chat_with_data():
    ...
    st.info("""
            ...
            1. Chat with text documents such as PDF, DOCX, XLSX, or TXT.
            2. Chat with content from web page or blog posts.
            3. Chat with content from a YouTube Video.
            """)
    ...
    with col1:
        input_option = st.radio(label="Select an input option", options=["Upload document(s)", "Paste an URL", "Paste a YouTube URL"])
        if input_option == "Upload document(s)":
            ...
            with col2:
                with st.expander('', expanded=True):
                    ...
                    with tab1:
                        if st.button("Reset Local Directory 🚮", use_container_width=True):
            ...
            with st.form("Input_WebURL"):
                ...
                submit_url = st.form_submit_button(label="Extract YouTube Video Transcript")

                if submit_url:
                    # Validate the YouTube Video URL
                    if validate_youtube_url(yt_url):
                        db, db_build_time = vector_db.run_db_build(input_type="yt_url", embeddings=gpt.embeddings, source_url=yt_url, db_persist=True)
            ...
            with col2:
                with st.expander('', expanded=True):
                    tab1, tab2, tab3 = st.tabs(["Manage_DB", "Video Details", "Watch Video"])
                    with tab1:
                        # Drop vector database
                        delete_vector_database()
                    if validate_youtube_url(yt_url):
    ...
    with st.form("QnA_Data"):
        input_query = st.text_input(label="Please enter the query that can be answered from available database",
    ...
    if submit_query:
        ...
        local_db = vector_db.load_local_db(embeddings=gpt.embeddings)
        if local_db is not None:
            with st.spinner("Retrieving response ..."):
                response = gpt.retrieval_qa(query=input_query,
