...
from pages.settings import page_config, custom_css, switch_main, delete_folder_contents, write_uploaded_file
...
if "valid_text_length" not in st.session_state:
    st.session_state.valid_text_length = False
...
def summary_ytvideo():
    ...
    with st.form("yt_video_summerize"):
        ...
        word_limit = st.slider(label="Select summary word limit", min_value=200, max_value=1000, step=100)
        submit_button = st.form_submit_button(label="Summarize Video", disabled=not st.session_state.valid_key)
...
def summary_document():
    ...
    with st.form("doc_summarize"):
        upload_document = st.file_uploader(label="Upload a document",
...
def summarization():
    ...
    input_option = col1.radio(label="Select an input option", options=["Paste plain text", "Paste an URL", "Paste an YouTube URL", "Upload a document"])
    with col2:
